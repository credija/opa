...
export default {
  saveConversationList(username, conversationList) {
    ...
    localStorage.setItem(
      btoa(`cached-conversation-list-${username}`),
      btoa(JSON.stringify(convListAux))
    );
  },
  loadConversationList(username, cachedRoster, store) {
    ...
    let cache = localStorage.getItem(
      btoa(`cached-conversation-list-${username}`)
    );
    ...
    if (cache !== null) {
      ...
      for (let i = 0; i < cache.length; i++) {
        let contactRef = cachedRoster.find(
          roster =>
            roster.username.toUpperCase() ===
            cache[i].contact.username.toUpperCase()
        );
          ...
          .then(res => {
            if (res !== undefined) {
              if (cachedConversationList[i].numUnreadMsgs !== 0) {
                const numUnreadConversation =
                  store.state.chat.numUnreadConversation;
                store.dispatch(
